...
Если \(k\) конечно, а \(\alpha\) --- образующая группы \(K^{\times}\), то \(K = k[\alpha]\).
...
\begin{remark}
Доказательство теоремы~\ref{thm:NormalBasis} позаимствовано из учебника~\cite[70]{milneFT_v5.10}.
\end{remark}
...
\begin{remark}
Я узнал об этом подходе к доказательству теоремы~\ref{thm:DedekindCharInd} из видео~\cite[лекция 5, 1:06:40]{Gorchinskiy_Galois_2022}.
\end{remark}
